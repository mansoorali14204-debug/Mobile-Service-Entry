...
function printJobSheet(id) {
    ...
    printJobSheetDocument(jobNumber);
}

function printJobSheetDocument(jobNumber) {
    const jobSheet = document.getElementById('jobSheet');
    if (!jobSheet) return;

    const printWindow = window.open('', '_blank', 'noopener,noreferrer,width=900,height=700');
    if (!printWindow) {
        showNotification('Please allow pop-ups to print a clean job sheet without site watermarks.', 'error');
        window.print();
        return;
    }
...
<head>
    <meta charset="UTF-8">
    <title>Job Sheet - ${jobNumber}</title>
    ...
    <style>
        ...
        .job-sheet { display: block !important; }
    </style>
</head>
<body>
    ${jobSheet.outerHTML}
...
function exportToExcel() {
    try {
        ...
        const filename = `ZenFix_Service_Entries_${dateStr}.xlsx`;
